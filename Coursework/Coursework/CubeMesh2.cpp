...
CubeMesh2::CubeMesh2(ID3D11Device* device, ID3D11DeviceContext* deviceContext, XMFLOAT3 size, XMFLOAT3 pos, int lresolution)
{
	resolution = lresolution;
	initBuffers(device, size, pos);
}

CubeMesh2::CubeMesh2(ID3D11Device* device, ID3D11DeviceContext* deviceContext, int lresolution)
{
	resolution = lresolution;
	initBuffers(device);
}
...
void CubeMesh2::initBuffers(ID3D11Device* device)
{
	VertexType* vertices;
...
void CubeMesh2::initBuffers(ID3D11Device* device, XMFLOAT3 size, XMFLOAT3 pos)
{
	VertexType* vertices;
	unsigned long* indices;
	D3D11_BUFFER_DESC vertexBufferDesc, indexBufferDesc;
	...
	// Vertex variables
	float yincrement = 2.0f * size.y / resolution;
	float xincrement = 2.0f * size.x / resolution;
	float ystart = 1.0f * size.y;
	float xstart = -1.0f * size.x;
	//UV variables
	float txu = 0.0f;
	float txv = 0.0f;
	float txuinc = 1.0f * size.x / resolution;	// UV increment
	float txvinc = 1.0f * size.y / resolution;
	...
	for (int y = 0; y < resolution; y++)	// for each quad in the y direction
	{
		for (int x = 0; x < resolution; x++)	// for each quad in the x direction
		{
			// Load the vertex array with data.
			//0
			vertices[v].position = XMFLOAT3(xstart, ystart - yincrement, -1.0f * size.z);  // Bottom left. -1. -1. 0
			...
			//1
			vertices[v].position = XMFLOAT3(xstart + xincrement, ystart, -1.0f * size.z);  // Top right.	1.0, 1.0 0.0
			vertices[v].position.x += pos.x;
			vertices[v].position.y += pos.y;
			vertices[v].position.z += pos.z;
			vertices[v].texture = XMFLOAT2(txu + txuinc, txv);
			...
			//2
			vertices[v].position = XMFLOAT3(xstart, ystart, -1.0f * size.z);  // Top left.	-1.0, 1.0
			vertices[v].position.x += pos.x;
			vertices[v].position.y += pos.y;
			vertices[v].position.z += pos.z;
			vertices[v].texture = XMFLOAT2(txu, txv);
			...
			//3
			vertices[v].position = XMFLOAT3(xstart + xincrement, ystart - yincrement, -1.0f * size.z);  // Bottom right.	1.0, -1.0, 0.0
			vertices[v].position.x += pos.x;
			vertices[v].position.y += pos.y;
			vertices[v].position.z += pos.z;
			vertices[v].texture = XMFLOAT2(txu + txuinc, txv + txvinc);
	...
	//back face
	// Vertex variables
	yincrement = 2.0f * size.y / resolution;
	xincrement = 2.0f * size.x / resolution;
	ystart = 1.0f * size.y;
	xstart = 1.0f * size.x;
	//UV variables
	txu = 0.0f;
	txv = 0.0f;
	txuinc = 1.0f * size.x / resolution;	// UV increment
	txvinc = 1.0f * size.y / resolution;
	...
	//right face
	yincrement = 2.0f * size.y / resolution;
	xincrement = 2.0f * size.z / resolution;
	ystart = 1.0f * size.y;
	xstart = -1.0f * size.z;
	//UV variables
	txu = 0.0f;
	txv = 0.0f;
	txuinc = 1.0f * size.z / resolution;	// UV increment
	txvinc = 1.0f * size.y / resolution;
	...
	for (int y = 0; y < resolution; y++)	// for each quad in the y direction
	{
		for (int x = 0; x < resolution; x++)	// for each quad in the x direction
		{
			// Load the vertex array with data.
			//0
			vertices[v].position = XMFLOAT3(1.0f * size.x, ystart - yincrement, xstart);  // Bottom left. -1. -1. 0
			vertices[v].position.x += pos.x;
			vertices[v].position.y += pos.y;
			vertices[v].position.z += pos.z;
			vertices[v].texture = XMFLOAT2(txu, txv + txvinc);
			...
			//2
			vertices[v].position = XMFLOAT3(1.0f * size.x, ystart, xstart + xincrement);  // Top right.	1.0, 1.0 0.0
			vertices[v].position.x += pos.x;
			vertices[v].position.y += pos.y;
			vertices[v].position.z += pos.z;
			vertices[v].texture = XMFLOAT2(txu + txuinc, txv);
			...
			//1
			vertices[v].position = XMFLOAT3(1.0f * size.x, ystart, xstart);  // Top left.	-1.0, 1.0
			vertices[v].position.x += pos.x;
			vertices[v].position.y += pos.y;
			vertices[v].position.z += pos.z;
			vertices[v].texture = XMFLOAT2(txu, txv);
			...
			//3
			vertices[v].position = XMFLOAT3(1.0f * size.x, ystart - yincrement, xstart + xincrement);  // Bottom right.	1.0, -1.0, 0.0
			vertices[v].position.x += pos.x;
			vertices[v].position.y += pos.y;
			vertices[v].position.z += pos.z;
			vertices[v].texture = XMFLOAT2(txu + txuinc, txv + txvinc);
	...
	//top face
	yincrement = 2.0f * size.z / resolution;
	xincrement = 2.0f * size.x / resolution;
	ystart = 1.0f * size.z;
	xstart = -1.0f * size.x;
	//UV variables
	...
	for (int y = 0; y < resolution; y++)	// for each quad in the y direction
	{
		for (int x = 0; x < resolution; x++)	// for each quad in the x direction
		{
			// Load the vertex array with data.
			//0
			vertices[v].position = XMFLOAT3(xstart, 1.0f * size.y, ystart - yincrement);  // Bottom left. -1. -1. 0
			vertices[v].position.x += pos.x;
			vertices[v].position.y += pos.y;
			vertices[v].position.z += pos.z;
			vertices[v].texture = XMFLOAT2(txu, txv + txvinc);
			...
			//2
			vertices[v].position = XMFLOAT3(xstart + xincrement, 1.0f * size.y, ystart);  // Top right.	1.0, 1.0 0.0
			vertices[v].position.x += pos.x;
			vertices[v].position.y += pos.y;
			vertices[v].position.z += pos.z;
			vertices[v].texture = XMFLOAT2(txu + txuinc, txv);
			...
			//1
			vertices[v].position = XMFLOAT3(xstart, 1.0f * size.y, ystart);  // Top left.	-1.0, 1.0
			vertices[v].position.x += pos.x;
			vertices[v].position.y += pos.y;
			vertices[v].position.z += pos.z;
			vertices[v].texture = XMFLOAT2(txu, txv);
			...
			//3
			vertices[v].position = XMFLOAT3(xstart + xincrement, 1.0f * size.y, ystart - yincrement);  // Bottom right.	1.0, -1.0, 0.0
			vertices[v].position.x += pos.x;
			vertices[v].position.y += pos.y;
			vertices[v].position.z += pos.z;
			vertices[v].texture = XMFLOAT2(txu + txuinc, txv + txvinc);
